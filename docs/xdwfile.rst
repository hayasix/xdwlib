...
``name``
    ...
    文字化けが生じます。対策として、 ``self.name_compat()`` を利用できます。

    オリジナルデータ名を保存 (更新) することはできません。
    必要であれば、いったん外部へ保存 (``save(self.name_compat())``)
    してから再度取り込む (``AttachmentList.append()``) 方法が考えられます。
...
``name_compat(encoding, errors='ignore')``
    オリジナルデータ名を、現在のシステムロケールにかかわらずコードページを
    指定して読み取ります。 ``encoding`` には作成時のロケール (日本語 Windows
    の場合は通常 ``'cp932'``) を指定します。

    このメソッドは、システムロケールを切り替える前に作成したオリジナルデータ
    名の文字化けを回避するために用意されています。
